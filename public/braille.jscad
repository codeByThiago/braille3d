...
function log(text) {
  if (OpenJsCad.log && debug) OpenJsCad.log(text);
}

function form_base() {
  if (parameters.plate_thickness <= 0.0) return new CSG();

  var dimensions = [
    parameters.form_distance / 2,
    parameters.line_height / 2,
    parameters.plate_thickness / 2,
  ];
  var offset = [
    parameters.form_distance / 2,
    -parameters.line_height / 2,
...
function ring(radius1, radius2, resolution) {
  ...
  var circle = CSG.Polygon.createFromPoints(points).translate([
    radius2 - radius1,
    0,
    0,
  ]);
...
function sized_dot() {
  if (master_dot == null) {
    ...
    dot = dot.scale([
      parameters.dot_diameter / 2,
      parameters.dot_diameter / 2,
      parameters.dot_height,
    ]);
    ...
    if (parameters.dot_shape == "smooth") {
      ...
      var base = CSG.cylinder({
        ...
        radius: ringRadius2 - ringRadius1,
        resolution: parameters.resolution,
      });
...
function dot(x, y) {
  var x_pos =
    (parameters.form_distance - parameters.dot_distance) / 2 +
    (x - 1) * parameters.dot_distance;
  var y_pos =
    -(parameters.line_height - parameters.dot_distance * 2) / 2 -
    (y - 1) * parameters.dot_distance;
...
function generate(text) {
  ...
  var offset = new CSG.Vector3D(
    parameters.plate_margin,
    -parameters.plate_margin,
    0
  );
  ...
  for (var c = 0; c < text.length; c++) {
    ...
    if (charCode > 0) textWidth = Math.max(textWidth, lineWidth);

    var characterDots = characterByCode(charCode);
    var position = offset.plus(
      new CSG.Vector3D(
        parameters.form_distance * (lineWidth - 1),
        parameters.line_height * -(numLines - 1),
        0
      )
    );
    for (var cp = 0; cp < characterDots.length; cp++)
      characterDots[cp] = characterDots[cp].translate([
        position.x,
        position.y,
        position.z,
      ]);
  ...
  var marginFactor = [
    (parameters.plate_margin * 2) / parameters.form_distance,
    (parameters.plate_margin * 2) / parameters.line_height,
  ];
  result = form_base().scale([
    textWidth + marginFactor[0],
    numLines + marginFactor[1],
    1,
  ]);
  ...
  if (parameters.reference_corner && parameters.plate_margin > 0) {
    var cornerCut = CSG.cube({
      center: [0, 0, 0],
      radius: [
        parameters.plate_margin,
        parameters.plate_margin / 2,
        parameters.plate_thickness,
      ],
    }).rotateZ(45);
    cornerCut = cornerCut.setColor(
      colorInside[0],
      colorInside[1],
      colorInside[2]
    );
  ...
  var dimensions = [
    textWidth * parameters.form_distance + parameters.plate_margin * 2,
    numLines * parameters.line_height + parameters.plate_margin * 2,
  ];
  ...
  if (parameters.stands) {
    ...
    var standCircle = CSG.cylinder({
      start: [0, 0, 0],
      end: [0, 0, standHeight],
      radius: standDiameter / 2,
      resolution: parameters.resolution,
    });
    var stand = CSG.cube({
      center: [0, parameters.plate_thickness / 2, standHeight / 2],
      radius: [
        bounds[1].x + standDiameter / 2,
        parameters.plate_thickness / 2,
        standHeight / 2,
      ],
    });
    stand = stand.union(
      standCircle.translate([
        bounds[0].x - standDiameter / 2,
        parameters.plate_thickness / 2,
        0,
      ])
    );
...
function getParameterDefinitions() {
  ...
  var parameterDefinitions = [
    ...
    {
      name: "upper",
      caption: "Permitir letras maiúsculas",
      type: "bool",
      initial: false,
    },
    {
    ...
    {
      name: "dot_height",
      caption: "Altura do ponto [0,5 - 0,8]: ",
      type: "float",
      initial: 0.7,
    },
    {
      name: "dot_diameter",
      caption: "Diâmetro da ponta [1,4 - 1,6]: ",
      type: "float",
      initial: 1.5,
    },

    {
      ...
      caption: "Espessura da placa (mm): ",
      type: "float",
      initial: 2.0,
    },
    {
      name: "plate_margin",
      caption: "Margem da placa (mm):	",
      type: "float",
      initial: 5.0,
    },

    {
      name: "reference_corner",
      caption: "Gerar canto de referência ?	",
      type: "bool",
      initial: true,
    },
    {
      name: "stands",
      caption: "Gerar suportes para impressão ? ",
      type: "bool",
      initial: true,
    },
    ...
    },
    {
      ...
      caption: "Punktform",
      type: "choice",
      values: ["sphere", "cylinder", "smooth"],
      captions: ["Halbkugel", "Zylinder", "Nahtlos"],
      initial: "smooth",
      visible: debug,
    },
    {
      name: "debug_dot",
      caption: "Punkt im Detail",
      type: "bool",
      initial: false,
      visible: debug,
    },
...
function main(params) {
  ...
  var result;
  if (parameters.debug_dot) result = sized_dot().scale([3, 3, 3]);
  else result = generate(parameters.text);
